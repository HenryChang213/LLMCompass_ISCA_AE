...
layernorm_TPUv3 = pd.read_csv(
    "real_hardware/layernorm_TPUv3.csv",
    header=None,
    names=["M", "N", "latency", "throughput"],
)
layernorm_TPUv3["throughput"] = (
    layernorm_TPUv3["throughput"].str.extract(r"(\d+\.?\d*)").astype(float)
)
layernorm_TPUv3.set_index(["M", "N"], inplace=True)
layernorm_TPUv3.sort_index(inplace=True)
...
for N in range(6, 16):
    ...
    N_list.append(N)
    # print(M,N)
    # print(layernorm_TPUv3.loc[(M, N), 'throughput'])
    throughput_TPU_list.append(layernorm_TPUv3.loc[(M, N), "throughput"].values[0])
