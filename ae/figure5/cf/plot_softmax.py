...
softmax_TPUv3 = pd.read_csv(
    "real_hardware/softmax_TPUv3.csv",
    header=None,
    names=["M", "N", "latency", "overhead", "throughput"],
)
softmax_TPUv3["throughput"] = (
    softmax_TPUv3["throughput"].str.extract(r"(\d+\.?\d*)").astype(float)
)
softmax_TPUv3.set_index(["M", "N"], inplace=True)
softmax_TPUv3.sort_index(inplace=True)
...
for N in range(6, 16):
    ...
    throughput_TPU_list.append(softmax_TPUv3.loc[(M, N), "throughput"].values[0])
