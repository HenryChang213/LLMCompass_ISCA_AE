...
gelu_TPUv3 = pd.read_csv(
    "real_hardware/gelu_TPUv3.csv", header=None, names=["M", "latency", "throughput"]
)
gelu_TPUv3["throughput"] = (
    gelu_TPUv3["throughput"].str.extract(r"(\d+\.?\d*)").astype(float)
)
...
for M in range(10, 30):
    ...
    M_list.append(M)
    # print(M,N)
    # print(gelu_TPUv3.loc[(M, N), 'throughput'])
    throughput_TPU_list.append(gelu_TPUv3[gelu_TPUv3["M"] == M]["throughput"].iloc[0])
...
print(f"Average error rate: {np.mean(avg_error_rate_0)}")
print(f"Average error rate: {np.mean(roofline_avg_error_rate_0)}")
